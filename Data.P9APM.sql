...
-- Below is P9.APM's version
	-- Updated to reflect EMP_NUMBER and removing names of ASSIGNEE/OWNERs from WHERE clause
	-- Updated to rolling 18 months for trunkated/"lite" APM App version
		-- The APD/full version will have original from 2020.03
SELECT 		
	o.OPP_ID
	,o.SUMMARY
	,o.OPP_STATUS
...
WHERE 
	...
	AND ((CONVERT(int,GETDATE()-1) - CONVERT(int,o.DATE_INSERTED)) / 548) < 1 -- Rolling 18 months. Updated by Tim for the 
	AND ((CONVERT(int,GETDATE()-1) - CONVERT(int,o.OPP_GENERATED_TIME)) / 548) < 1 -- Rolling 18 months 
--	AND o.DATE_INSERTED BETWEEN '2020-03-01 00:00:00.001' AND GETDATE() -- APD version will use this date frame
--	AND o.OPP_GENERATED_TIME BETWEEN '2020-03-01 00:00:00.001' AND GETDATE() -- APD version will use this date frame
	AND o.EMP_NUMBER_ASSIGNEE NOT IN ('4118541','4118551','analyst@profitect.com','9999999','1959518','1432914','2634721','1950877','1440795','1445805','jim.hare'
		...
		,'1185165','4546396','4234682','1863183','1325015','4255465','4256323','1448552','4546413','4546391','4546246','4546399','2589783','4748046')
	...
	AND o.ASSIGNEE <> 'Unknown Unknown' -- Semetrius confirmed removal of terminated ASSIGNEEs 10.31.2024
GROUP BY 
	o.OPP_ID,o.SUMMARY,o.OPP_STATUS,o.OPP_RESOLUTION,o.OPP_VALUE,o.ASSIGNEE,o.PATTERN_NAME,o.OPPORTUNITY_ENTITY
	,o.EMP_NUMBER_ASSIGNEE,o.[OWNER],o.EMP_NUMBER_OWNER,o.LABELS,o.DATE_INSERTED,o.OPP_GENERATED_TIME,o.ETHICSPOINT_CASE_NUMBER
...
-- Below is P9.APM's version
	-- Updated to reflect EMP_NUMBER and removing names of ASSIGNEE/OWNERs from WHERE clause
	-- Updated to add NAVEX (ETHICSPOINT_CASE_NUMBER) 
	-- Date now from Sept 2020
SELECT 		
	o.OPP_ID
	,o.SUMMARY
	,o.OPP_STATUS
...
WHERE 
	o.OPP_STATUS NOT IN ('HISTORY','Deleted')
	AND o.SUMMARY NOT LIKE 'Welcome to%'
	AND o.DATE_INSERTED BETWEEN '2020-09-01 00:00:00.001' AND GETDATE() -- Tim confirmed 6.30.2021 that FQ2 2020 (Feb 2020) is not needed
	AND o.OPP_GENERATED_TIME BETWEEN '2020-09-01 00:00:00.001' AND GETDATE() -- Was going to replace DATE_INSERTED with this but loss of 4k rows >> RAY confirmed that we can add this filter to get rid of unneccesary historical data 9.28.2021
	AND o.EMP_NUMBER_ASSIGNEE NOT IN ('4118541','4118551','analyst@profitect.com','9999999','1959518','1432914','2634721','1950877','1440795','1445805','jim.hare'
		,'1224862','2982027','2251383','1224862','1222234','2488189','1224384','1223749','2013702','1062954','2223799','4043932','2246468','2860864','IITeam'
...
-- Below was replaced on 20231023 bc 'SCOTT MCMULLEN' + 'MICHAEL OADDAMS' were missing from alerts to be reported
	-- ALSO Updated to reflect EMP_NUMBER and removing names of ASSIGNEE/OWNERs from WHERE clause
...
-- Below was initial P9APM Query. Replaced with current bc I wanted to add the store and district number (Just in case it is needed)
SELECT 		
	o.OPP_ID
	,o.SUMMARY
	,o.OPP_STATUS
...
-- Below was previous Comments >> STAR SCHEMA 
-- Replaced with above to reflect EMP_NUMBER and removing names of ASSIGNEE/OWNERs from WHERE clause
